...
if [ ! -x evaluatorRT/PST ]; then
	mkdir evaluatorRT/PST
fi

for occup in "0.75" "0.5" "0.25"
do

	OUTDIR=evaluatorRT/PST/$occup
	...
	do
		...
		echo EVALUATE
		for table in outputs/realTimeScaledPST/$occup/*$circtab*.tab
		do
			tablecirc=$(echo $table | sed s/_table/""/ | sed  s/.tab/""/ | sed s:outputs/realTimeScaledPST/$occup/:"":)
